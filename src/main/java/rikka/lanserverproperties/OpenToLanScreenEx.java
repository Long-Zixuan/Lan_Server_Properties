...
public class OpenToLanScreenEx {
	...
	private static Component wifiAndIpAddressLabel = Component.translatable("");
	...
	public static void postDraw(Screen gui, Font textRenderer, GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (mc.hasSingleplayerServer()) {
			...
			Component wifiAndIpAddressLabel = Component.translatable("WIFI:"+getWiFiName()+"   IP:"+getIPAdress());
			//wifiAndIpAddressLabel.setText("WIFI:"+getWiFiName()+"   IP:"+getIPAdress());
			guiGraphics.drawString(textRenderer, wifiAndIpAddressLabel, gui.width / 2 - 155, gui.height - 76, 10526880);
		}
	}
